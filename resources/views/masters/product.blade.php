                    ...
                    <form method="POST" action="{{ route('product.store') }}">
                        ...
                        <div class="row gy-4">
                            <div class="col-xxl-4 col-md-6">
                                <div>
                                    <label for="name" class="@error('name') is-invalid @enderror form-label">Product Name</label>
                            ...
                            <table role="grid" class="gridjs-table" style="height: auto;" id="results">
                                <thead class="gridjs-thead">
                                    <tr class="gridjs-tr">
                                        ...
                                        <th data-column-id="serialno" class="gridjs-th" style="width: 120px;">Product</th>
                                        <th data-column-id="date" class="gridjs-th" style="width: 120px;">Description</th>
                                        <th data-column-id="date" class="gridjs-th" style="width: 120px;">UOM</th>
                                ...
                                <tbody class="gridjs-tbody">
                                    ...
                                    <tr class="gridjs-tr" >
                                        ...
                                        <td data-column-id="serialno" class="gridjs-td">{{ $product->name }}</td>
                                        <td data-column-id="date" class="gridjs-td">{{ $product->description }}</td>
                                        <td data-column-id="date" class="gridjs-td">{{ $product->uom }}</td>
                                        <td data-column-id="isActive" class="gridjs-td">
                                            <div class="form-check form-switch">
                                                <input class="form-check-input active-switch" type="checkbox" role="switch" id="activeSwitch_{{ $product->id }}" data-id="{{ $product->id }}" {{ $product->isActive ? 'checked' : '' }}>
                                            </div>    
                                        </td>
                                    </tr>
                                    @endforeach
...
<script>
    ...
    document.querySelectorAll('.active-switch').forEach(function(switchBtn) {
        switchBtn.addEventListener('change', function() {
            ...
            fetch(`/product/${productId}/updateActive`, {
                method: 'POST',
                headers: {
